...
它保持 agent 侧的 ACP 原生协议不变，因此同一个 Hermes 实例可以承接任何通过 `stdio` 说 ACP 的 agent，并通过当前的 Telegram 适配器对外提供服务，后续也可以继续扩展到更多聊天平台。
...
| Discord | TODO | 已有通道抽象和 `DiscordAdapter` 占位实现，但事件接入、消息发送/编辑、配置和运行时接线尚未完成。当前版本只启动 Telegram。 |
...
- `src/adapters/discord/DiscordAdapter.ts` 目前只是占位。
- `ChannelAdapter` 和 orchestrator 的结构已经为 Discord 做好了预留。
- 当前实现说明见 [docs/discord-v2.md](./docs/discord-v2.md)。
...
启动 Hermes 后，在 Telegram 中和你的 bot 对话，并通过下面的命令创建 session：
...
- `src/adapters/discord/`: Discord 占位实现
